...
import { getProfessionalWorkingHours, WorkingHours, isDateWorkingDay, generateAvailableTimeSlots, isSlotAvailable } from '../utils/workingHours';
...
export default function RechercheProPage() {
  ...
  const handleRdvSubmit = async (e: React.FormEvent) => {
    ...
    // Vérification de la disponibilité du créneau principal (prévention double-booking)
    const isMainSlotAvailable = await isSlotAvailable(
      selectedProfessionnel.user_id, 
      selectedDate, 
      selectedTime
    );
    
    if (!isMainSlotAvailable) {
      showToast('Ce créneau n\'est plus disponible. Veuillez choisir un autre horaire.', 'error');
      // Recharger les créneaux disponibles
      await loadAvailableTimes(selectedDate);
      return;
    }

    // Vérification de la disponibilité des créneaux alternatifs
    const alternativeSlots = rdvFormData.creneauxAlternatifs.filter(creneau => creneau.date && creneau.heure);
    for (const creneau of alternativeSlots) {
      const isAltSlotAvailable = await isSlotAvailable(
        selectedProfessionnel.user_id,
        creneau.date,
        creneau.heure
      );
      
      if (!isAltSlotAvailable) {
        showToast(`Le créneau alternatif du ${creneau.date} à ${creneau.heure} n'est plus disponible. Veuillez le modifier ou le supprimer.`, 'error');
        return;
      }
    }
    ...
    try {
      ...
      // Appeler l'API de test pour créer le rendez-vous avec auth
      console.log('🧪 TEST: Utilisation de l\'API de test avec auth');
  ...
  const isWorkingDate = (dateString: string): boolean => {
    console.log('🔍 isWorkingDate: Vérification pour', dateString);
    ...
    console.log('📅 Date créée (UTC):', date);
    console.log('📅 Jour de la semaine (UTC):', date.getUTCDay());
    
    const isWorking = isDateWorkingDay(selectedProfWorkingHours, date);
    console.log('✅ Est un jour de travail:', isWorking);
    return isWorking;
  };

  // Charger les créneaux disponibles pour une date donnée
  const loadAvailableTimes = async (date: string) => {
    if (!date || !selectedProfessionnel) {
      console.log('❌ loadAvailableTimes: Pas de date ou professionnel sélectionné');
      setAvailableTimes([]);
      return;
    }
    
    console.log('🔍 loadAvailableTimes: Début du chargement des créneaux');
    console.log('📅 Date sélectionnée:', date);
    console.log('👨‍⚕️ Professionnel:', selectedProfessionnel.prenom, selectedProfessionnel.nom);
    console.log('⏰ Horaires de travail:', selectedProfWorkingHours);
    console.log('⏰ Structure complète working_hours:', JSON.stringify(selectedProfWorkingHours, null, 2));
    ...
    try {
      ...
      if (!isWorkingDate(date)) {
        console.log('❌ loadAvailableTimes: Ce n\'est pas un jour de travail');
        setAvailableTimes([]);
        return;
      }
      
      console.log('✅ loadAvailableTimes: C\'est un jour de travail');
      ...
      console.log('⏱️ Durée de consultation:', consultationDuration, 'minutes');
      
      // Utiliser les horaires de travail du professionnel et exclure les créneaux réservés
      // Corriger le problème de fuseau horaire en créant la date en UTC pour éviter les décalages
      const [year, month, day] = date.split('-').map(Number);
      const dateObj = new Date(Date.UTC(year, month - 1, day)); // Créer la date en UTC
      console.log('📅 Date string originale:', date);
      console.log('📅 Date object corrigée (UTC):', dateObj);
      console.log('📅 Jour de la semaine (UTC):', dateObj.getUTCDay(), '(0=dimanche, 5=vendredi)');
      
      const times = await generateAvailableTimeSlots(
        selectedProfWorkingHours, 
        dateObj, 
        selectedProfessionnel.user_id,
