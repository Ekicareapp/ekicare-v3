...
export default function RendezVousPage() {
  ...
  const updateAppointmentStatus = async (appointmentId: string, action: string, data?: any) => {
    try {
      ...
      if (action === 'accept') {
        ...
        showToast('Rendez-vous replanifié avec succès', 'success');
      } else if (action === 'add_compte_rendu') {
  ...
  const handleOpenGPS = (appointment: Appointment) => {
    ...
    // En production, on pourrait utiliser les coordonnées du client
  ...
  const getMenuActions = (appointment: Appointment) => {
    ...
    switch (appointment.status) {
      ...
      case 'confirmed':
        // À venir : Actions client disponibles
        actions.push(
          {
            label: 'Fiche client',
            icon: <User className="w-4 h-4" />,
            onClick: () => {
              handleViewClient(appointment);
            }
          },
          {
            label: 'Appeler',
      ...
      case 'rescheduled':
        // Replanifiés : En attente de revalidation du proprio
        actions.push({
          label: 'Replanifier à nouveau',
          icon: <RotateCcw className="w-4 h-4" />,
          onClick: () => {
            handleReplanifier(appointment);
          }
